...
async def verify_token(token: str) -> User:
    ...
    Returns:
        User: The user corresponding to the verified token.

    Raises:
        HTTPException: If the token is invalid or the user is not found.
    """
    try:
        payload: dict[str, Any] = jwt.decode(token=token, key=SECRET_KEY, algorithms=[ALGORITHM])  # type: ignore
        logger.debug(payload)
        user:User = User.objects.get(email=payload.get("sub"))  # type: ignore
        return user
